...
## Running DDR-opt on BARN

You can run the [DDR-opt](https://github.com/ZJU-FAST-Lab/DDR-opt) trajectory optimization stack on BARN:

1. **Single workspace**: Build both BARN packages and DDR-opt packages in one catkin workspace (see `barn_ddr_bridge/README.md` for the list of required packages).
2. **Run with DDR-opt**:
   ```bash
   python3 run.py --world_idx 0 --nav_stack ddr_opt
   ```
   Use `--nav_stack ddr_opt` to use DDR-opt; default is `dwa` (move_base with DWA). The bridge package `barn_ddr_bridge` converts BARN laser and odometry to DDR-opt inputs and DDR-opt commands to Jackal `cmd_vel`.
