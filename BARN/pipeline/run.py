...
if __name__ == "__main__":
    ...
    parser.add_argument('--nav_stack', type=str, default="dwa", choices=["dwa", "ddr_opt"],
                        help="Navigation stack: 'dwa' (move_base DWA) or 'ddr_opt' (DDR-opt trajectory optimization)")
    ...
    use_ddr_opt = (args.nav_stack == "ddr_opt")
    if use_ddr_opt:
        # DDR-opt: bridge + plan_manager + mpc (no move_base)
        nav_stack_process = subprocess.Popen([
            'roslaunch', 'barn_ddr_bridge', 'ddr_opt_barn.launch',
            'gui:=' + ("true" if args.gui else "false")
        ])
        time.sleep(8)  # wait for DDR-opt nodes and SDF map to be ready
        # Send goal to DDR-opt via /move_base_simple/goal (PoseStamped in odom frame)
        from geometry_msgs.msg import PoseStamped, Quaternion
        goal_pub = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size=1, latch=True)
        goal_msg = PoseStamped()
        goal_msg.header.frame_id = 'odom'
        goal_msg.header.stamp = rospy.Time.now()
        goal_msg.pose.position.x = INIT_POSITION[0] + GOAL_POSITION[0]
        goal_msg.pose.position.y = INIT_POSITION[1] + GOAL_POSITION[1]
        goal_msg.pose.position.z = 0.0
        goal_msg.pose.orientation = Quaternion(0, 0, 0, 1)
        goal_pub.publish(goal_msg)
        rospy.loginfo("DDR-opt goal published: (%.2f, %.2f) in odom", goal_msg.pose.position.x, goal_msg.pose.position.y)
    else:
        # Default: move_base with DWA
